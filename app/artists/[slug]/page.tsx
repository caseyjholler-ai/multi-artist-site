...
export default async function ArtistPage({
  ...
  const isDigital = (artwork: Artwork) => artwork.type === 'digital'
    ...
    <>
      <style>{`
        ...
        .sb-header {
          text-align: center;
          padding-bottom: 20px;
          border-bottom: 2px solid #2a2320;
          margin-bottom: 28px;
        }
      ...
      <div className="sb-root">
        ...
        <div className="sb-content">
          <div className="sb-header">
            <div className="sb-title">{artist.name}&apos;s Flash</div>
          ...
          <div className="sb-sheet">
            {artworks && artworks.map((artwork, index) => (
              <>
                <div key={artwork.id} className="sb-card" id={`card-${artwork.id}`}>
                  <button
                    className="frame-btn"
                    onClick={() => {
                      const card = document.getElementById(`card-${artwork.id}`)
                      const allCards = document.querySelectorAll('.sb-card')
                      allCards.forEach(c => { if (c.id !== `card-${artwork.id}`) c.classList.remove('open') })
                    ...
                    <div className="expand-body">
                      ...
                      <div className="expand-meta">
                        ...
                        <div className="expand-specs">
                          {isDigital(artwork) ? 'Digital Download' : 'Original'}
                        </div>
                      </div>
                      <button className="buy-btn">
                        {isDigital(artwork) ? 'Buy Print File' : 'Purchase Original'}
                      </button>
                    </div>
                ...
                {index < (artworks.length - 1) && (
                  <div key={`divider-${artwork.id}`} className="sb-divider">✦</div>
                )}
              </>
            ))}

            {(!artworks || artworks.length === 0) && (
              <div style={{textAlign:'center', color:'#9a8060', fontSize:'12px', letterSpacing:'3px', textTransform:'uppercase', padding:'40px 0'}}>
        ...
        <footer className="sb-footer">
          ...
          <div className="footer-name">{artist.name}</div>
          <div className="footer-bio">{artist.bio}</div>
        </footer>
      </div>
    </>
  )
